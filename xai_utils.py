...
def preprocess_image(_learner, _pil_image):
    ...
    test_dl = _learner.dls.test_dl(
        [img_pil_rgb], with_labels=False, device=_learner.dls.device
    )
...
def find_target_layer(_learner):
    ...
    try:
        if hasattr(_learner.model[0][7][-1], "conv3"):
            return _learner.model[0][7][
                -1
            ]  # Returnează blocul, nu conv3, pentru compatibilitate mai largă
    except:
        pass
...
@st.cache_data
def generate_manual_gradcam_image(
    _pil_image, _learner, predicted_class_idx, image_identifier_for_cache
):
    if _learner is None:
    ...
    try:
        ...
        if isinstance(processed_item_pil, torch.Tensor):
                ...
                try:
                    ...
                    if not isinstance(processed_item_pil, Image.Image):
                        if hasattr(processed_item_pil, "convert"):
                            pil_img_for_tv = Image.fromarray(
                                np.array(processed_item_pil.convert("RGB"))
                            )
            ...
            st.error(
                f"XAI Error: Unexpected type for 'processed_item_pil': {type(processed_item_pil)}"
            )
            return _pil_image
        ...
        if not isinstance(img_tensor, torch.Tensor):
            st.error(
                f"XAI Error: Image could not be converted to a tensor. 'img_tensor' is currently {type(img_tensor)}."
            )
            return _pil_image
        ...
        x_batch_for_norm = TensorImage(
            img_tensor_batch.float()
        )  # se asigură că e TensorImage pentru normalizare
        ...
        try:
            ...
            st.error(
                f"Eroare la aplicarea normalizării directe cu imagenet_stats: {e_explicit_norm}. Vizualizarea (Grad-CAM) poate fi incorect."
            )
        ...
        try:
            ...
            elif hasattr(_learner.model, "0") and hasattr(
                _learner.model[0], "_modules"
            ):
                ...
                for i in range(len(body_modules) - 1, -1, -1):
                    ...
                    if (
                        ...
                        if any(
                            ...
                            for m in last_submodule_in_block.modules()
                        ):
            ...
            if target_layer is None:
                potential_layers = [
                    m
                    for m in _learner.model.modules()
                    if isinstance(m, torch.nn.Conv2d)
                ]
                if potential_layers:
                    target_layer = potential_layers[-1]
        except Exception as e_layer_sel:
            potential_layers = [
                m for m in _learner.model.modules() if isinstance(m, torch.nn.Conv2d)
            ]
            if potential_layers:
                target_layer = potential_layers[-1]
        ...
        if target_layer is None:
            st.warning(
                TEXT_RO_XAI["xai_placeholder"]
                + " (Layer țintă invalid pentru vizualizare(Grad-CAM))"
            )
            return _pil_image

        grad_cam_instance = ManualGradCAM(_learner.model, target_layer)
        heatmap_np = grad_cam_instance(
            img_tensor_batch_normalized, index=predicted_class_idx
        )
        ...
        heatmap_resized = cv2.resize(
            heatmap_np, (img_pil_rgb.width, img_pil_rgb.height)
        )  # img_pil_rgb folosit pentru dimensiuni inițiale
        ...
        superimposed_img_np = cv2.addWeighted(
            edge_map_np, 0.6, heatmap_colored_rgb, 0.4, 0
        )
...
def generate_library_cam_visualization(
    _learner, _pil_image, predicted_class_idx, cam_method_class, use_edge_map=True
):
    ...
    visualization = show_cam_on_image(
        background_img_np, grayscale_cam_resized, use_rgb=True
    )
...
def generate_guided_cam(_learner, _pil_image, predicted_class_idx, cam_method_class):
    ...
    if cam_method_class == ManualGradCAM:
        ...
        cam_instance = cam_method_class(
            model=_learner.model, target_layers=[target_layer]
        )
        targets = [ClassifierOutputTarget(predicted_class_idx)]
        result = cam_instance(
            input_tensor=input_tensor,
            targets=targets if cam_method_class != EigenCAM else None,
        )
