...
from prediction import get_prediction, get_xai_visualization
from db_utils import (
    get_gallery_from_db,
    remove_from_gallery_db,
    save_image,
    add_to_gallery_db,
    get_map_locations_from_db,
    remove_from_map_db,
    add_to_map_db,
    add_feedback_db,
    get_style_details,
...
def main_app_page(
    st_session_state, current_learner, TEXT_RO, KNOWN_ARCHITECTURAL_STYLES_list
):
    st.title(TEXT_RO["app_title"])
    st.markdown(
        "### (Detector Orientat spre Recunoașterea Esteticii Latente și Identificare Arhitecturală)"
    )
    ...
    with col1:
        ...
        if uploaded_file is not None:
            if (
                ...
                or not st_session_state.current_pil_image
            ):  # Also check if image is not loaded
            ...
            try:
                ...
                st_session_state.current_image_bytes_for_display = (
            ...
            if (
                st_session_state.current_pil_image is not None
                or st_session_state.current_filename is not None
            ):
                st_session_state.current_pil_image = None
                st_session_state.current_predictions = None
                st_session_state.current_filename = None
    ...
    with col2:
        ...
        if uploaded_file and st_session_state.current_pil_image is not None:
            button_key_suffix = (
                f"{st_session_state.username}_{st_session_state.current_filename}"
                if st_session_state.current_filename
                else (
                    uploaded_file.file_id
                    if hasattr(uploaded_file, "file_id")
                    else str(random.randint(0, 100000))
                )
            )

            if st.button(
                TEXT_RO["analyze_button"],
                type="primary",
                key=f"analyze_btn_{button_key_suffix}",
            ):
                if current_learner is None:
                    st.error(
                        "Modelul nu este încărcat. Vă rugăm selectați un model valid din bara laterală și reîncercați."
                    )
                else:
                    with st.spinner(TEXT_RO["analyzing_spinner"]):
                        try:
                            pil_image_for_predict = st_session_state.current_pil_image
                            pred_class_str, pred_idx_tensor, outputs_tensor = (
                                current_learner.predict(pil_image_for_predict)
                            )
                            ...
                            for i in range(k):
                                ...
                                period_info = TEXT_RO.get(
                                    "period_info_default", "Perioadă Est.: Necunoscută"
                                )
                                region_info = TEXT_RO.get(
                                    "region_info_default", "Regiune: Necunoscută"
                                )
                                more_info_text_db = (
                                    "Mai multe informații nu sunt disponibile."
                                )
                                if style_details_db:
                                    period_info = style_details_db.get(
                                        "period", period_info
                                    )
                                    region_info = style_details_db.get(
                                        "region", region_info
                                    )
                                    more_info_text_db = style_details_db.get(
                                        "details_text", more_info_text_db
                                    )
                                    details_link_item = style_details_db.get(
                                        "details_link", details_link_item
                                    )
                            ...
                            st_session_state.feedback_submitted_for_current_image = (
                                False
                            )
                            st_session_state.current_image_id_in_db = None
            ...
            if st_session_state.current_predictions:
                ...
                high_conf_predictions = [
                    (s, c, pi)
                    for s, c, pi in zip(
                        styles_only, confidences_only, pred_indices_only
                    )
                    if c >= 0.40
                ]
                ...
                if not high_conf_predictions:
                    ...
                    primary_display_style = TEXT_RO["style_amestec"].format(
                        style1=style1_hc, style2=style2_hc
                    )
                    display_confidence_message = TEXT_RO["mixed_style_message"]
                    if conf1_hc >= 0.40:
                        show_xai_for.append(high_conf_predictions[0])
                    if conf2_hc >= 0.40:
                        show_xai_for.append(high_conf_predictions[1])
                ...
                if show_xai_for:
                    ...
                    # --- Butonul de generare ---
                    button_label = (
                        f"Generează: {selected_cam_name}"
                        if cam_method_class
                        else f"Generează: {viz_type_key}"
                    )
                    if st.button(
                        button_label, key=f"generate_button_{button_key_suffix}"
                    ):
                            ...
                            with st.expander(expander_title, expanded=True):
                                ...
                                with st.spinner(spinner_text):
                                    ...
                                    if xai_image:
                                        ...
                                        st.warning(
                                            "Vizualizarea nu a putut fi generată."
                                        )
                ...
                if show_predictions and predictions:
                    ...
                    for i, pred_info in enumerate(predictions):
                        ...
                        period = pred_info.get(
                            "period",
                            TEXT_RO.get(
                                "period_info_default", "Perioadă Est.: Necunoscută"
                            ),
                        )
                        region = pred_info.get(
                            "region",
                            TEXT_RO.get("region_info_default", "Regiune: Necunoscută"),
                        )
                        more_info_text = pred_info.get(
                            "more_info_text", "Detalii indisponibile."
                        )
                        ...
                        st.markdown(
                            f"**{TEXT_RO['prediction_confidence']}:** {confidence*100:.2f}%"
                        )
                        st.markdown(f"*{period}* | *{region}*")

                        with st.expander(
                            f"{TEXT_RO['more_info_link'].format(style=style)}"
                        ):
                            st.markdown(more_info_text)
                            st.markdown(
                                f"[Link Info]({details_link})", unsafe_allow_html=True
                            )
                        st.markdown("---")
                ...
                if not st_session_state.feedback_submitted_for_current_image:
                    ...
                    with feedback_form:
                        ...
                        correct_prediction = feedback_cols_buttons[
                            0
                        ].form_submit_button(TEXT_RO["feedback_yes"])
                        incorrect_prediction_btn = feedback_cols_buttons[
                            1
                        ].form_submit_button(TEXT_RO["feedback_no"])

                        current_suggestion_box_id = (
                            f"suggestion_for_{button_key_suffix}"
                        )
                        if (
                            "show_suggestion_box_for_id" not in st_session_state
                            or st_session_state.show_suggestion_box_for_id
                            != current_suggestion_box_id
                        ):
                        ...
                        if incorrect_prediction_btn:
                            ...
                            st_session_state.show_suggestion_box_for_id = (
                                current_suggestion_box_id
                            )

                        if (
                            st_session_state.get("show_suggestion_box_active", False)
                            and st_session_state.get("show_suggestion_box_for_id")
                            == current_suggestion_box_id
                        ):
                            suggested_style_input = st.text_input(
                                TEXT_RO["feedback_suggest_style_prompt"],
                                key=f"suggest_{button_key_suffix}",
                            )
                            if st.form_submit_button(TEXT_RO["feedback_submit_button"]):
                                add_feedback_db(
                                    st_session_state.username,
                                    st_session_state.current_image_id_in_db,
                                    st_session_state.current_filename,
                                    primary_display_style,
                                    False,
                                    (
                                        suggested_style_input
                                        if suggested_style_input
                                        else None
                                    ),
                                )
                        ...
                        if incorrect_prediction_btn and not st_session_state.get(
                            "show_suggestion_box_active", False
                        ):
                            st_session_state.show_suggestion_box_active = True
                            st_session_state.show_suggestion_box_for_id = (
                                current_suggestion_box_id
                ...
                if st_session_state.current_image_bytes_for_display:
                    ...
                    unique_key_suffix_save = (
                        f"{st_session_state.username}_{st_session_state.current_filename}"
                        if st_session_state.current_filename
                        else f"{st_session_state.username}_save_area"
                    )

                    user_notes = st.text_area(
                        TEXT_RO["notes_prompt"], key=f"notes_{unique_key_suffix_save}"
                    )
                    address_input = st.text_input(
                        TEXT_RO["address_prompt"],
                        key=f"address_{unique_key_suffix_save}",
                    )

                    col_save1, col_save2 = st.columns(2)
                    with col_save1:
                        if st.button(
                            TEXT_RO["add_gallery_button"],
                            key=f"add_gallery_btn_{unique_key_suffix_save}",
                        ):
                            ...
                            if image_path:
                                ...
                                gallery_items_db = get_gallery_from_db(
                                    st_session_state.username, text_ro_config=TEXT_RO
                                )
                                is_duplicate_db = any(
                                    item["filename"]
                                    == st_session_state.current_filename
                                ...
                                if not is_duplicate_db:
                                    image_db_id = add_to_gallery_db(
                                        st_session_state.username,
                                        st_session_state.current_filename,
                                        image_path,
                                        st_session_state.current_predictions,
                                        user_notes,
                                        address_input if address_input else None,
                                        TEXT_RO,
                                    )
                                    if image_db_id:
                                        st_session_state.current_image_id_in_db = (
                                        ...
                                        st.success(
                                            TEXT_RO["gallery_added_success"].format(
                                                filename=st_session_state.current_filename
                                            )
                                        )
                                        st.session_state.image_added_to_gallery = True
                                    else:
                                        st.warning(
                                            TEXT_RO["gallery_already_exists"].format(
                                                filename=st_session_state.current_filename
                                            )
                                        )
                    ...
                    with col_save2:
                        if st.button(
                            TEXT_RO["add_map_button"],
                            key=f"add_map_btn_{unique_key_suffix_save}",
                        ):
                            if not st.session_state.get(
                                "image_added_to_gallery", False
                            ):
                                st.error(
                                    "Trebuie să adăugați imaginea în galerie înainte de a o adăuga pe hartă."
                                )
                            elif address_input:
                                try:
                                    ...
                                    with st.spinner(
                                        f"Se geocodează '{address_input}'..."
                                    ):
                                        location = geolocator.geocode(
                                            address_input, timeout=10
                                        )

                                    if location:
                                        lat, lon = location.latitude, location.longitude
                                        map_locations_db = get_map_locations_from_db(
                                            st_session_state.username,
                                            text_ro_config=TEXT_RO,
                                        )
                                        is_duplicate_map_db = any(
                                            item["address"] == address_input
                                            and item["filename"]
                                            == st_session_state.current_filename
                                            for item in map_locations_db
                                        )

                                        if not is_duplicate_map_db:
                                            thumb_img = Image.open(
                                                io.BytesIO(
                                                    st_session_state.current_image_bytes_for_display
                                                )
                                            )
                                            ...
                                            image_preview_path = save_image(
                                                thumb_byte_arr.getvalue(),
                                                f"thumb_{st_session_state.current_filename}",
                                                st_session_state.username,
                                                is_thumbnail=True,
                                            )
                                            ...
                                            if image_preview_path:
                                                ...
                                                st.success(
                                                    TEXT_RO["map_added_success"].format(
                                                ...
                                                st.error(
                                                    "Nu s-a putut salva thumbnai; pentru hartă."
                                                )
                                        else:
                                            st.warning(
                                                TEXT_RO[
                                                    ...
                                                    filename=f"{st_session_state.current_filename} la adresa {address_input}"
                                                )
                                            )  # Clarify warning
                                    else:
                                        st.error(
                                            TEXT_RO["map_address_not_found"].format(
                                                address=address_input
                                            )
                                        )
                                except ImportError:
                                    st.error(
                                        "Biblioteca Geopy nu este instalată. Rulați `pip install geopy`."
                                    )
                                except GeocoderTimedOut:
                                    st.error(
                                        TEXT_RO["map_geocoding_error"].format(
                                            e="Timeout"
                                        )
                                    )
                                    ...
                                    st.error(
                                        TEXT_RO["map_geocoding_error"].format(
                                            e="Serviciu indisponibil"
                                        )
                                    )
                                except Exception as e_geo:
                                    st.error(
                                        TEXT_RO[
                                            "map_unexpected_geocoding_error"
                                        ].format(e=e_geo)
                                    )
                            else:
...
def gallery_page(st_session_state, TEXT_RO):
    ...
    gallery_items = get_gallery_from_db(
        st_session_state.username, text_ro_config=TEXT_RO
    )
    ...
    all_styles_in_gallery = sorted(
        list(
            set(
                item["primary_style"] for item in gallery_items if item["primary_style"]
            )
        )
    )
    ...
    amestec_prefix = TEXT_RO.get("style_amestec", "Amestec {style1} + {style2}").split(
        " "
    )[0]
    ...
    if any(
        style.startswith(amestec_prefix) for style in all_styles_in_gallery if style
    ):
        filter_options.append(TEXT_RO.get("amestec_option", "Amestec de Stiluri"))

    specific_styles_in_gallery = [
        s
        for s in all_styles_in_gallery
        if s and s != neidentificat_label and not s.startswith(amestec_prefix)
    ]
    ...
    if selected_style_filter == TEXT_RO["all_styles_option"]:
        ...
        filtered_gallery = [
            ...
            if item["primary_style"] == neidentificat_label
        ]
    elif selected_style_filter == TEXT_RO.get("amestec_option", "Amestec de Stiluri"):
        filtered_gallery = [
            item
            for item in gallery_items
            if item["primary_style"]
            and item["primary_style"].startswith(amestec_prefix)
        ]
    else:
        filtered_gallery = [
            item
            for item in gallery_items
            if item["primary_style"] == selected_style_filter
        ]
    ...
    num_columns = st.slider(
        TEXT_RO["gallery_columns_slider"],
        1,
        5,
        3,
        key=f"gallery_cols_db_ui_{st_session_state.username}",
    )
    ...
    for index, item in enumerate(filtered_gallery):
        ...
        with cols[col_index]:
            if item["image_bytes"]:
                st.image(
                    item["image_bytes"],
                    caption=f"Fișier: {item.get('filename', 'N/A')}",
                )
            else:
                st.warning(
                    f"Imaginea pentru {item.get('filename', 'N/A')} nu a putut fi încărcată."
                )
            ...
            if primary_style_gal == neidentificat_label:
                ...
                st.markdown(
                    f"**{TEXT_RO['gallery_top_style']}:** {primary_style_gal} ({primary_confidence_gal*100:.1f}%)"
                )

            expander_key_gallery = (
                f"gallery_expander_{item['id']}_{st_session_state.username}"
            )
            with st.expander(TEXT_RO["gallery_view_analysis_notes"]):
                num_preds_to_show_gallery = min(3, len(item["predictions"]))
                for i, pred_info in enumerate(
                    item["predictions"][:num_preds_to_show_gallery]
                ):
                    style_det = pred_info["style"]
                    confidence_det = pred_info["confidence"]
                    link_det = pred_info.get("link", "#")
                    period_det = pred_info.get(
                        "period",
                        TEXT_RO.get(
                            "period_info_default", "Perioadă Est.: Necunoscută"
                        ),
                    )
                    region_det = pred_info.get(
                        "region",
                        TEXT_RO.get("region_info_default", "Regiune: Necunoscută"),
                    )
                    ...
                    st.markdown(
                        f"[Detalii Wikipedia]({link_det})", unsafe_allow_html=True
                    )
                ...
                if item["address"]:
                    st.markdown(
                        f"**{TEXT_RO['gallery_saved_address']}** {item['address']}"
                    )

            remove_btn_key_gallery_ui = (
                f"remove_gallery_db_ui_{item['id']}_{st_session_state.username}"
            )
            if st.button(
                TEXT_RO["gallery_remove_button"].format(
                    filename=item.get("filename", "Imagine")[:15]
                ),
                key=remove_btn_key_gallery_ui,
            ):

                if remove_from_gallery_db(item["id"], st_session_state.username):
                    st.success(
                        f"'{item.get('filename', 'Imagine')}' a fost șters/ștearsă din galerie."
                    )
                    st.rerun()
                else:
...
def map_favorites_page(st_session_state, TEXT_RO):
    ...
    map_locations = get_map_locations_from_db(
        st_session_state.username, text_ro_config=TEXT_RO
    )
    ...
    try:
        ...
        all_styles_on_map = sorted(
            list(set(item["style"] for item in map_locations if item["style"]))
        )
        neidentificat_label_map = TEXT_RO.get("style_neidentificat", "Neidentificat")
        amestec_prefix_map = TEXT_RO.get(
        ...
        if neidentificat_label_map in all_styles_on_map:
            filter_options_map.append(
                TEXT_RO.get("neidentificat_option", "Neidentificat")
            )
        if any(
            style.startswith(amestec_prefix_map) for style in all_styles_on_map if style
        ):
            filter_options_map.append(
                TEXT_RO.get("amestec_option", "Amestec de Stiluri")
            )

        specific_styles_on_map = [
            s
            for s in all_styles_on_map
            if s
            and s != neidentificat_label_map
            and not s.startswith(amestec_prefix_map)
        ]
        ...
        elif selected_style_filter_map == TEXT_RO.get(
            "neidentificat_option", "Neidentificat"
        ):
            df_to_display = df_locations[
                df_locations["style"] == neidentificat_label_map
            ]
        elif selected_style_filter_map == TEXT_RO.get(
            "amestec_option", "Amestec de Stiluri"
        ):
            df_to_display = df_locations[
                df_locations["style"].str.startswith(amestec_prefix_map, na=False)
            ]
        else:
            df_to_display = df_locations[
                df_locations["style"] == selected_style_filter_map
            ]

        if df_to_display.empty:
            st.warning(
                TEXT_RO["no_locations_for_style"].format(
                    style=selected_style_filter_map
                )
            )
            return

        map_center = (
            [df_to_display["latitude"].mean(), df_to_display["longitude"].mean()]
            if not df_to_display.empty
            else [45.9432, 24.9668]
        )  # Romania center
        ...
        st_folium(
            m, width=725, height=500, key=f"folium_map_{st_session_state.username}"
        )
        ...
        if not df_to_display.empty:
            location_options_map_ui = {
                f"{item['filename']} @ {item['address']} (Stil: {item['style']})": item[
                    "id"
                ]
        ...
        if location_options_map_ui:
            ...
            if st.button(
                TEXT_RO["map_remove_selected_button"],
                key=f"remove_map_button_db_ui_{st_session_state.username}",
            ):
                if not locations_to_remove_display_texts:
                    st.info(TEXT_RO["map_remove_none_selected"])
                else:
                    ids_to_remove = [
                        location_options_map_ui[text]
                        for text in locations_to_remove_display_texts
                        if text in location_options_map_ui
                    ]
                    if ids_to_remove:

                        removed_count = remove_from_map_db(
                            ids_to_remove, st_session_state.username
                        )
                        if removed_count > 0:
                            st.success(
                                TEXT_RO["map_remove_success"].format(
                                    count=removed_count
                                )
                            )
                            st.rerun()
        ...
        st.image(
            "https://placehold.co/800x400/E0E0E0/757575?text=Hartă+Interactivă+-+Necesită+Instalare",
            caption=TEXT_RO["map_libs_missing_image_caption"],
        )
