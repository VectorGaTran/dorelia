...
from werkzeug.security import generate_password_hash, check_password_hash # Importă funcțiile corecte
...
def save_image(image_bytes, original_filename, username, is_thumbnail=False):
    ...
    safe_original_filename_stem = "".join(c if c.isalnum() else "_" for c in Path(original_filename).stem)
...
def add_to_gallery_db(username, filename, image_path, predictions_list, notes, address, TEXT_RO_SESSION):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    ...
    if predictions_list and len(predictions_list) > 0:
        ...
        if not high_conf_preds:
            primary_style = TEXT_RO_SESSION.get("style_unidentified", "Neidentificat")
            primary_confidence = predictions_list[0]['confidence'] if predictions_list else 0.0
            ...
            primary_style = TEXT_RO_SESSION.get("style_mix", "Amestec {style1} + {style2}").format(style1=style1, style2=style2)
            primary_confidence = (high_conf_preds[0]['confidence'] + high_conf_preds[1]['confidence']) / 2
    ...
    try:
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (username, filename, image_path, primary_style, primary_confidence, predictions_json, notes, address))
        conn.commit()
        ...
        cursor.execute("SELECT id FROM gallery WHERE image_path = ? AND username = ?", (image_path, username)) # verifică și pentru utilizator
...
def get_gallery_from_db(username, text_ro_config=None):
    ...
    for item in items:
        ...
        gallery_data.append({
            'id': item[0], 'filename': item[1], 'image_path': item[2],
            'primary_style': item[3] if item[3] else current_text_ro.get("style_unidentified", "Neidentificat"),
            'primary_confidence': item[4] if item[4] is not None else 0.0,
            'predictions': predictions_list,
            'notes': item[6], 'address': item[7],
...
def remove_from_gallery_db(item_id, username):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT image_path FROM gallery WHERE id = ? AND username = ?", (item_id, username))
        result = cursor.fetchone()
        if result:
            image_path_to_delete = Path(result[0])
            cursor.execute("DELETE FROM gallery WHERE id = ? AND username = ?", (item_id, username))
            conn.commit()
            if conn.total_changes > 0:
                cursor.execute("SELECT COUNT(*) FROM gallery WHERE image_path = ? AND username = ?", (str(image_path_to_delete), username))
                gallery_references = cursor.fetchone()[0]
                cursor.execute("SELECT COUNT(*) FROM map_locations WHERE image_preview_path = ? AND username = ?", (str(image_path_to_delete), username))
                map_references = cursor.fetchone()[0]

                if image_path_to_delete.exists() and gallery_references == 0 and map_references == 0 : 
                    try:
                        image_path_to_delete.unlink()
                    except Exception as e_file_delete:
                        st.warning(f"Imaginea a fost scoasă din baza de date, dar a eșuat ștergerea fișierului: {e_file_delete}")
                return True
            return False
        return False
    except Exception as e:
...
def add_to_map_db(username, address, latitude, longitude, style, image_preview_path, filename):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    try:
        cursor.execute("""
    ...
    for item in items:
        ...
        if item[5] and Path(item[5]).exists(): 
            try:
                ...
                print(f"Eroare la citirea thumbnail-ului pentru hartă: {item[5]}, {e_img}")

        locations_data.append({
            'id': item[0], 'address': item[1],
            'latitude': item[2], 'longitude': item[3],
            'style': item[4] if item[4] else current_text_ro.get("style_unidentified", "Neidentificat"),
...
def remove_from_map_db(location_ids, username): 
    ...
    try:
        for loc_id in location_ids:
            cursor.execute("SELECT image_preview_path FROM map_locations WHERE id = ? AND username = ?", (loc_id, username))
            result = cursor.fetchone()
            thumb_path_to_delete = None
            if result and result[0]:
                thumb_path_to_delete = Path(result[0])

            cursor.execute("DELETE FROM map_locations WHERE id = ? AND username = ?", (loc_id, username))
            if cursor.rowcount > 0:
                removed_count += 1
                if thumb_path_to_delete and thumb_path_to_delete.exists():
                    cursor.execute("SELECT COUNT(*) FROM map_locations WHERE image_preview_path = ? AND username = ?", (str(thumb_path_to_delete), username))
                    map_references = cursor.fetchone()[0]
                    if map_references == 0:
                        try:
                            thumb_path_to_delete.unlink()
                        except Exception as e_file_del_map:
                            st.warning(f"Locația a fost scoasă din baza de date, dar ștergerea thumbnail-ului a eșuat: {e_file_del_map}")
        conn.commit()
    except Exception as e:
...
def add_feedback_db(username, image_db_id, image_filename, predicted_style, is_correct, user_corrected_style=None, comment=None):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    try:
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (username, image_db_id, image_filename, predicted_style, 1 if is_correct else 0, user_corrected_style, comment))
        conn.commit()
    except Exception as e:
...
def get_style_details(style_name):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT period_info, region_info, more_info_text, more_info_link FROM style_details WHERE style_name = ?", (style_name,))
    details = cursor.fetchone()
    conn.close()
    if details:
        return {"period": details[0], "region": details[1], "details_text": details[2], "details_link": details[3]}
    return None

def add_or_update_style_details(style_name, period_info, region_info, more_info_text, more_info_link):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute('''
