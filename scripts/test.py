...
def main():  # Întregul tău cod va merge în această funcție
    ...
    try:
        ...
        print(
            "Imaginile vor fi redimensionate și normalizate automat după configurația modelului."
        )
    ...
    if not TEST_IMAGE_FOLDER.is_dir():
        print(
            f"Eroare: directorul cu imagini pentru testare nu a fost găsit sau nu este un director la '{TEST_IMAGE_FOLDER}'."
        )
    ...
    if test_image_files and len(test_image_files) > 0:
        num_example_images = min(
            15, len(test_image_files)
        )  # Limitează numărul de imagini pentru exemplu
        individual_test_image_paths = test_image_files[:num_example_images]
        print(
            f" Au fost selectate primele {len(individual_test_image_paths)} imagini din '{TEST_IMAGE_FOLDER}' pentru exemple de predicții individuale:"
        )
    else:
        print(f" Nu au fost găsite imagini în {TEST_IMAGE_FOLDER}.")
    ...
    if individual_test_image_paths:
        ...
        for img_path in individual_test_image_paths:
            if not img_path.is_file():
                print(
                    f" Avertisment: Fișierul imagine nu a fost găsit la {img_path}, se omite."
                )
                continue
            try:
