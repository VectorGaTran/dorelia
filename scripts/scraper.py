        ...
        for page_id, page_info in pages.items():
            if "imageinfo" in page_info:
                ...
                if not (
                    ...
                    print(
                        f"  Se sare imagine de format non-standard '{mime_type}' pentru {file_page_title}"
                    )
                    return False

                original_filename = page_info.get(
                    "title", f"{style_name_for_filename}_{int(time.time()*1000)}.jpg"
                ).replace("File:", "")
...
def get_category_members(
    ...
    processed_files,
    download_count_for_style,
    style_save_dir,
    style_name_for_file,
):
    if (
        category_title in processed_categories
        or download_count_for_style["count"] >= MAX_IMAGES_PER_STYLE
    ):
        return

    print(
        f"Categoria procesată: {category_title} (Images downloaded for style: {download_count_for_style['count']}/{MAX_IMAGES_PER_STYLE})"
    )
    ...
    for keyword in AVOID_CATEGORY_KEYWORDS:
        if keyword.lower() in category_title.lower():
            print(
                f"  S-a sărit categoria deoarece conținea cuvântul'{keyword}': {category_title}"
            )
            return
    ...
    while True:
        if download_count_for_style["count"] >= MAX_IMAGES_PER_STYLE:
            print(
                f"  Numărul de imagini descărcate maxim a fost atins pentru stilul {style_name_for_file}."
            )
            break
        ...
        try:
            ...
            for member in members:
                ...
                if member_type == "file":
                    if member_title not in processed_files:
                        if download_image_from_api(
                            member_title, style_save_dir, style_name_for_file
                        ):
            ...
            try:
                ...
                print(
                    f"Reîncarcare eșuată pentru categoria: {category_title}: {e_retry}"
                )
                break
        except requests.exceptions.RequestException as e:
    ...
    for style_key, root_categories in CATEGORIES_TO_SCRAPE.items():
        ...
        for root_cat in root_categories:
            if images_downloaded_for_style["count"] < MAX_IMAGES_PER_STYLE:
                ...
                print(
                    f"S-a atins numărul maxim de imagini pentru stilul {style_key} înainte de procesarea tuturor categoriilor."
                )
                break

        print(
            f"===== S-a terminat scrape-ul pentru stilul: {style_key}. Număr imagini descărcate: {images_downloaded_for_style['count']} ====="
        )
        if images_downloaded_for_style["count"] < MAX_IMAGES_PER_STYLE:
            print(
                f"Au fost găsite și descărcate mai puține imagini ({MAX_IMAGES_PER_STYLE}) admise pentru stilul {style_key}."
            )
